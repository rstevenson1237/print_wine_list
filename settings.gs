 ...
 *   Images         — logo and footer image files
 *   Colors         — primary brand color and text color
 *   Welcome        — title page toggle, date, three paragraphs
...
/**
 * Returns the active brand name.
 * @returns {string}
 */
function getBrandName() {
  return PropertiesService.getUserProperties().getProperty(PROP_KEYS.BRAND_NAME) || 'BYGONE';
}

/**
 * Returns the active brand preset object.
 * @returns {Object}
 */
...
/**
 * Loads a brand preset, writing all its values into PropertiesService.
 * This overwrites ALL current settings with the preset defaults.
 *
...
/**
 * Saves just the brand name without loading full preset.
 * @param {string} brandName
 * @returns {Object}
 */
function saveBrandName(brandName) {
  PropertiesService.getUserProperties().setProperty(PROP_KEYS.BRAND_NAME, brandName);
...
// ============================================================================
// Getters — Heading Styles (unchanged logic, uses brand fallback)
// ============================================================================
...
// ============================================================================
// Getters — Wine Entry, Images, Colors, Welcome (brand-aware fallbacks)
// ============================================================================
...
// ============================================================================
// Aggregate Getter (for Settings dialog load)
// ============================================================================
...
function saveHeadingStyleProp(type, group, prop, value) {
  try {
    PropertiesService.getUserProperties().setProperty(headingKey_(type, group, prop), value.toString());
 ...
 * @param {Object} pageObj { sizePreset, width, height, marginTop, marginBottom, marginInner, marginOuter, pageBuffer, frontMatterPages }
 * @returns {Object}
 */
...
function saveFontFile(base64Data, fileName) {
  ...
  return saveFileToDrive_(base64Data, fileName, getMimeForExtension(ext));
}

function saveImageFile(slot, base64Data, fileName) {
  ...
  if (result.success) {
    PropertiesService.getUserProperties().setProperty(propKey, fileName);
  }
  return result;
}
...
function saveColor(slot, value) {
  ...
  PropertiesService.getUserProperties().setProperty(propKey, value);
